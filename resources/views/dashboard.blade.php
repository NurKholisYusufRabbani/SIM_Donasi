        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <p class="text-sm text-gray-600 dark:text-gray-400 mb-6">Anda login sebagai: {{ ucfirst(Auth::user()->role) }}</p>
                    ...
                    @if(Auth::user()->role === 'admin')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                            <div class="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg shadow">
                                ...
                                <p class="text-2xl font-bold">Rp {{ number_format($totalDonationAmountAccepted ?? 0, 2, ',', '.') }}</p>
                                <p class="text-sm text-gray-500 dark:text-gray-400">{{ $acceptedDonations ?? 0 }} donasi diterima dari {{ $totalDonations ?? 0 }} total pengajuan.</p>
                            </div>
                            {{-- ... (kartu statistik lainnya) ... --}}
                             <div class="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg shadow">
                                <h4 class="font-semibold text-gray-700 dark:text-gray-200">Total Dana Didistribusikan</h4>
                                <p class="text-2xl font-bold">Rp {{ number_format($totalDistributionAmount ?? 0, 2, ',', '.') }}</p>
                            </div>
                            <div class="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg shadow">
                                ...
                                @if(($pendingDonations ?? 0) > 0)
                                    <a href="{{ route('donations.index', ['status' => 'pending']) }}" class="text-sm text-indigo-600 dark:text-indigo-400 hover:underline">Lihat & Verifikasi</a>
                                @endif
                            </div>
                            <div class="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg shadow">
                                <h4 class="font-semibold text-gray-700 dark:text-gray-200">Total Pengguna</h4>
                                <p class="text-2xl font-bold">{{ $totalUsers ?? 0 }}</p>
                                <a href="{{ route('users.index') }}" class="text-sm text-indigo-600 dark:text-indigo-400 hover:underline">Kelola Pengguna</a>
                            </div>
                            <div class="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg shadow">
                                ...
                                <a href="{{ route('donors.index') }}" class="text-sm text-indigo-600 dark:text-indigo-400 hover:underline">Kelola Donatur</a>
                            </div>
                            <div class="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg shadow">
                                ...
                                <a href="{{ route('beneficiaries.index') }}" class="text-sm text-indigo-600 dark:text-indigo-400 hover:underline">Kelola Penerima</a>
                            </div>
                        </div>

                        {{-- GRAFIK DONASI DITERIMA PER BULAN --}}
                        <div class="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg shadow mb-6">
                            <h4 class="text-md font-semibold mb-3 text-gray-700 dark:text-gray-200">Tren Nominal Donasi Diterima (6 Bulan Terakhir)</h4>
                            ...
                            <table class="min-w-full text-sm divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-4 py-2">{{$donation->donor->user->username ?? 'N/A'}}</td>
                                        <td class="px-4 py-2">Rp {{number_format($donation->amount,0,',','.')}}</td>
                                        <td class="px-4 py-2">{{\Carbon\Carbon::parse($donation->date)->isoFormat('D MMM YY')}}</td>
                                        <td class="px-4 py-2">
                                            <a href="{{ route('donations.show', $donation->id) }}" class="text-indigo-600 dark:text-indigo-400 hover:underline">Detail</a>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
                        ...
                        @else
                        <p class="text-sm text-gray-500 dark:text-gray-400">Tidak ada donasi yang menunggu verifikasi saat ini.</p>
                        @endif

                        {{-- Akses Cepat Manajemen yang sudah ada sebelumnya --}}
                        <h4 class="text-md font-semibold mb-2">Akses Cepat Manajemen:</h4>
                        <div class="flex flex-wrap gap-4">
                           {{-- ... link akses cepat ... --}}
                            <a href="{{ route('donations.index') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Manajemen Donasi</a>
                            <a href="{{ route('distributions.index') }}" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">Manajemen Distribusi</a>
                            <a href="{{ route('users.index') }}" class="bg-purple-500 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded">Manajemen Pengguna</a>
                            <a href="{{ route('donors.index') }}" class="bg-teal-500 hover:bg-teal-700 text-white font-bold py-2 px-4 rounded">Manajemen Donatur</a>
                            <a href="{{ route('beneficiaries.index') }}" class="bg-orange-500 hover:bg-orange-700 text-white font-bold py-2 px-4 rounded">Manajemen Penerima</a>
                        </div>
